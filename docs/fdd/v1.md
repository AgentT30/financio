...
Has visibility into all users’ data for debugging (disabled by default in V1).
...
Password ≥ 8 chars, no strict complexity.
...
- Toggle card number visibility (masked ↔ full with spaces)
...
- CVV field preservation in edit mode (shows ••• placeholder)
...
### 4.2.5 Fixed Deposits

Fixed deposit accounts with maturity tracking.

**Fields:**

| Field | Type | Notes |
|-------|------|-------|
| Name / Nickname | Text | FD display name |
| Institution | Text | Bank name |
| FD Number | String | FD account/certificate number |
...
| Maturity Amount | Decimal | Expected maturity value, optional |
| Auto Renewal | Boolean | Auto-renew on maturity |
| Opening Balance | Decimal | Same as principal |
| Currency | INR (fixed) | — |
...
- Track maturity date
- View FD detail
- Calculate expected returns (future)
- Maturity reminders (future)
...
### Common Behavior Across All Account Types
...
### 4.2.7 Account Integration & Selection
...
- Examples:
  ...
  - "📱 Google Pay (UPI Wallet)" [future]
  - "💵 Wallet Cash (Cash)" [future]

**Emoji Indicators:**
| Emoji | Account Type | Notes |
|-------|--------------|-------|
| 🏦 | Bank Accounts | Savings, Checking, Current, Salary |
| 💳 | Credit Cards | All credit card types |
| 📱 | Digital Wallets | UPI apps, mobile wallets (future) |
| 💵 | Cash Accounts | Physical cash (future) |
...
| Credit/Debit | Enum | Credit (+) or Debit (−) |
...
**Account Selection:**
- Combined dropdown with emoji indicators (🏦 💳 📱 💵)
- Format: "[Emoji] Account Name (Institution/Type)"
...
- **Expense on Credit Card:** Balance becomes MORE negative (debt increases)
  - Example: Balance -₹5,000, spend ₹2,000 → New balance -₹7,000
  - Available credit decreases by ₹2,000
  - Amount owed increases by ₹2,000
- **Income on Credit Card** (cashback/refund): Balance becomes LESS negative (debt decreases)
  - Example: Balance -₹5,000, refund ₹1,000 → New balance -₹4,000
  - Available credit increases by ₹1,000
  - Amount owed decreases by ₹1,000
...
Debit = Expense → reduces account balance (or increases credit card debt).

Credit = Income → increases account balance (or reduces credit card debt).
...
Debit = Expense → reduces account balance.

Credit = Income → increases account balance.
...
- Emoji indicators for account types (🏦 💳 📱 💵)
- Format: "[Emoji] Account Name (Institution/Type)"
- GenericForeignKey implementation for polymorphic references

**Supported Transfer Scenarios:**
1. 🏦 Bank → 🏦 Bank (standard inter-account transfer)
2. 🏦 Bank → 💳 Credit Card (bill payment - reduces credit card debt)
3. 💳 Credit Card → 🏦 Bank (refund/reversal - increases credit card debt)
4. 💳 Credit Card → 💳 Credit Card (balance transfer between cards)
5. 📱 Wallet → 🏦 Bank (future - wallet to bank)
6. 💵 Cash → 🏦 Bank (future - cash deposit)
...
Example:
- Bank balance: ₹50,000
- Credit card balance: -₹10,000 (owe ₹10,000)
- Transfer ₹5,000 from Bank → Credit Card
- New bank balance: ₹45,000
- New credit card balance: -₹5,000 (owe ₹5,000)
- New available credit: Credit limit - ₹5,000 (instead of credit limit - ₹10,000)
...
From Account ≠ To Account (validates both ContentType ID and object ID for GenericForeignKey).
...
- Same account check: Compares ContentType ID AND object ID (allows BankAccount ID=4 → CreditCard ID=4)
...
Tracks user’s fixed income deposits.
...
| Total Accounts | Count of accounts with breakdown: "X Banks • Y Cards" |
...
1. **Net Worth Card**
   ...
   - Shows in Indian number format without decimals (e.g., ₹10,63,833)
   - Non-clickable (informational only)

2. **Total Accounts Card**
   - Shows breakdown: "X Banks • Y Cards"
   - Example: "5 Accounts" with subtitle "3 Banks • 2 Cards"
...
**Quick Actions:**
- **Add Account** button → Shows choice modal with options:
  - "Add Bank Account" → Links to bank account create form
  - "Add Credit Card" → Links to credit card create form
- **Record Transaction** button → Links to transaction create form
- **Transfer Money** button → Links to transfer create form
...
User fills form → Validates input.
...
- /db_data → Postgres persistent data.
- /media → user-uploaded receipts (future).
...
- **8 Test Suites:**
  ...
  2. Transfer Tests (bank↔credit card, credit card↔credit card)
