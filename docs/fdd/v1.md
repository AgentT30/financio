...
| Institution | Text | Issuing bank |
| Card Number | Encrypted Text | Full card number (encrypted) |
| Card Number (Last 4) | String | Last 4 digits for display |
| Card Type | Enum | Visa / Mastercard / RuPay / Amex |
| Credit Limit | Decimal | Total credit limit, optional |
| Billing Day | Integer | Day of month (1-31), optional |
| Due Day | Integer | Payment due day (1-31), optional |
| Expiry Date | Date | Card expiry, optional |
| Opening Balance | Decimal | Initial balance (usually 0 or negative) |
| Currency | INR (fixed) | — |
| Status | Enum | Active / Archived |
| Notes | Text | Optional |
| Color | Hex Code | Visual customization |
...
- Calculate available credit (limit - current balance)
- View current balance (negative = amount owed)
- View card detail & linked transactions
- Mask card number
...
- Balance Tracking: Materialized balance table for fast lookups
- User Scoping: All accounts belong to a user
- Visual Customization: Color coding and optional pictures/icons
- Title Case Storage: Names and institutions stored in Title Case
...
Used to move funds between user’s accounts.
