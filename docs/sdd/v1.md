...
User
 ...
 │    └─< [Account metadata e.g., FD/Loan pointer if used]
 ├─< Category
 ├─< JournalEntry
 │    └─< Posting (many)   -- postings reference Account
 ├─< Instrument
 │    └─< Trade (many)
...
JournalEntry 1:N Posting (posting references Account)

User 1:N Instrument/Trade/Holding

Trade optionally references a FundingAccount (Account)
...
accounts — user-owned accounts (type enum: savings, credit_card, wallet, cash, fd, loan). Masked account_id/number for display.

categories — hierarchical categories (parent_id), type (expense/income/transfer/fee), color.

transactions — user-visible transaction log. Fields: datetime_ist, sign (credit/debit), amount, account_id, method_type, purpose (free-text), category_id, created_at, updated_at, deleted_at, journal_entry_id (nullable).
...
postings — postings of each journal entry: journal_entry_id, account_id, amount (signed), currency (INR), memo.

instruments — investments: name, symbol, ISIN, type (stock/mf), notes.

trades — trade records: user_id, instrument_id, trade_type (buy/sell/dividend/fees), units, price, fees, funding_account_id (nullable), date_time, note.

holdings — derived aggregation per instrument (units, avg_cost); can be maintained as cached/denormalized table.

fd_records — FD metadata: principal, rate, compound_freq, start_date, maturity_date, maturity_amount.

loans — loan metadata: lender, principal, rate, emi_amount, start_date, due_day.

balances_materialized — materialized or cached balance per account (account_id, balance_amount, last_updated_at). This is the primary table for fast balance lookups.
