...
- No charts in V1.
- 2–3 users and a few thousand transactions in year one.
...
User
├─< SavingsAccount
│ ├─ SavingsAccountBalance (1:1)
│ └─< DebitCard (many:1)
├─< CreditCard
│ └─ CreditCardBalance (1:1)
├─< DigitalWallet
│ └─ DigitalWalletBalance (1:1)
├─< CashAccount
│ └─ CashAccountBalance (1:1)
├─< FixedDeposit
│ └─ FixedDepositBalance (1:1)
├─< LoanAccount
│ └─ LoanAccountBalance (1:1)
├─< Transaction (UI log, GenericFK to any account)
├─< Transfer (links two transactions/journal entries)
├─< Category
├─< JournalEntry
│ └─< Posting (many) -- postings use GenericFK to any account type
├─< Instrument
│ └─< Trade (many, GenericFK to funding account)
│ └─ creates/affects Holding
└─< ActivityLog
...
Transaction 0/1 → JournalEntry (linked)
...
Below are the core tables and primary purpose—fields described conceptually (not exhaustive columns).

**Core Tables:**

- **users** — user profiles, creds (hashed password), preferences (theme), session details.

**Account Tables (Specialized by Type):**

- **savings_accounts** — Savings/checking accounts. Fields: user_id, name, subtype (savings/checking/salary), institution, branch_name, ifsc_code, account_number (encrypted), account_number_last4, customer_id, opening_balance, currency, opened_on, status, notes, color, picture, created_at, updated_at.

- **debit_cards** — Debit cards linked to bank accounts. Fields: user_id, bank_account_id, name, card_type (visa/mastercard/rupay/amex/other), card_number (encrypted), card_number_last4, cvv (encrypted), expiry_date, status, notes, color, picture, created_at, updated_at.

- **credit_cards** — Credit card accounts. Fields: user_id, name, institution, card_number (encrypted), card_number_last4, card_type (visa/mastercard/rupay/amex), credit_limit, billing_day, due_day, expiry_date, opening_balance, currency, status, notes, color, created_at, updated_at.

- **digital_wallets** — Digital wallet accounts (UPI apps, mobile wallets). Fields: user_id, name, wallet_type (upi/wallet/other), provider, linked_mobile, upi_id, opening_balance, currency, opened_on, status, notes, color, created_at, updated_at.

- **cash_accounts** — Physical cash holdings. Fields: user_id, name, cash_type (wallet/home/other), location, opening_balance, currency, status, notes, color, created_at, updated_at.

- **fixed_deposits** — FD accounts. Fields: user_id, name, institution, fd_number, principal_amount, interest_rate, tenure_months, maturity_date, maturity_amount, auto_renewal, opening_balance, currency, opened_on, status, notes, color, created_at, updated_at.

- **loan_accounts** — Loan accounts. Fields: user_id, name, loan_type (personal/home/auto/education/other), lender_name, loan_account_number, principal_amount, interest_rate, emi_amount, tenure_months, emi_day, start_date, opening_balance, currency, status, notes, color, created_at, updated_at.

**Balance Tables (One per Account Type):**

- **bank_account_balances** — Materialized balances for bank accounts. Fields: account_id (PK, FK to bank_accounts), balance_amount, last_posting_id, updated_at.

- **credit_card_balances** — Materialized balances for credit cards. Fields: account_id (PK, FK to credit_cards), balance_amount, last_posting_id, updated_at.

- **digital_wallet_balances** — Materialized balances for wallets (future). Fields: account_id (PK, FK to digital_wallets), balance_amount, last_posting_id, updated_at.

- **cash_account_balances** — Materialized balances for cash (future). Fields: account_id (PK, FK to cash_accounts), balance_amount, last_posting_id, updated_at.

- **fixed_deposit_balances** — Materialized balances for FDs (future). Fields: account_id (PK, FK to fixed_deposits), balance_amount, last_posting_id, updated_at.

- **loan_account_balances** — Materialized balances for loans (future). Fields: account_id (PK, FK to loan_accounts), balance_amount, last_posting_id, updated_at.

**Balance Calculation Robustness:**

- Each account model has `get_current_balance()` method that queries balance table
- Falls back to `opening_balance` if balance record doesn't exist
- Dashboard and list views use `get_current_balance()` instead of direct aggregation
- Prevents errors when balance records are missing or deleted
- Balance recalculation command: `python manage.py recalculate_balances`

**Transaction & Ledger Tables:**

- **categories** — hierarchical categories (parent_id), type (expense/income/transfer/fee), color.

- **transactions** — user-visible transaction log. Fields: datetime_ist, sign (credit/debit), amount, account_content_type (ContentType), account_object_id (generic FK to any account type), method_type, purpose (free-text), category_id, created_at, updated_at, deleted_at, journal_entry_id (nullable).

- **transfers** — (optional table) links two transaction ids OR links two postings, representing a user-visible transfer.

- **journal_entries** — journal entries representing the accounting event (zero-sum per currency). Fields: user_id, occurred_at, memo, created_at.

- **postings** — postings of each journal entry. Uses GenericForeignKey to reference any account type. Fields: journal_entry_id, account_content_type, account_object_id, amount (signed), currency (INR), posting_type (debit/credit), memo.

**Investment Tables:**

- **instruments** — investments: name, symbol, ISIN, type (stock/mf), notes.

- **trades** — trade records: user_id, instrument_id, trade_type (buy/sell/dividend/fees), units, price, fees, funding_account_content_type, funding_account_object_id (generic FK to account), date_time, note.

- **holdings** — derived aggregation per instrument (units, avg_cost); can be maintained as cached/denormalized table.

**Audit:**

- **activity_logs** — CRUD actions: user_id, object_type, object_id, action, diff_json, ip, created_at.
...
3. **Balance Materialization:** Each account type has its own balance table (one-to-one relationship) for fast balance lookups without complex joins.
   ...
   - bank_account_balances → BankAccount
   - credit_card_balances → CreditCard
...
5. **Account Selection UI:** Transaction and Transfer forms use a unified dropdown showing all account types with emoji indicators:
   ...
   - 🏦 Bank Accounts (Savings, Checking, Current, Salary)
   - 💳 Credit Cards (Visa, Mastercard, RuPay, Amex)
   - 📱 Digital Wallets (future - UPI apps, mobile wallets)
   - 💵 Cash Accounts (future - physical cash)
...
6. **Credit Card Debt Tracking:**
   ...
   - Available Credit = Credit Limit + Current Balance
     - Example: Limit ₹50,000, Balance -₹5,000 → Available ₹45,000
   - Amount Owed = abs(min(balance, 0))
   - Bill payments (Bank → Credit Card) reduce debt (balance becomes less negative)
...
7. **Dashboard Net Worth Calculation:**
   ...
   - Uses `get_current_balance()` method for each account (graceful degradation)
...
8. **Indian Number Formatting:**
   ...
   - Custom template filter `indian_format` in core/templatetags/indian_numbers.py
   - Formats numbers in Indian numbering system (X,XX,XXX)
   - Accepts optional decimals parameter: `{{ value|indian_format:0 }}` for integers
   - Default is 2 decimal places: `{{ value|indian_format }}` or `{{ value|indian_format:2 }}`
   - Applied across dashboard, account lists, detail pages, and transaction views
   - Dashboard stats show integers (no decimals) for cleaner display
   - Transaction amounts show 2 decimal places for precision

9. Ledger design — minimal double-entry
...
- Use a canonical numeric sign: postings store signed amounts in INR, with positive meaning increase asset/equity depending on account type is complex—so prefer storing amount as signed and keep direction field per posting:
...
- A user transaction (Expense Debit) creates:
  ...
  - Posting 1: Debit: Category ledger (or Expense ledger) — we represent categories as analytic tags, not ledger accounts; to keep minimal double-entry, we will:
  - Posting A (Debit): Expense placeholder posting (synthetic ledger account per category or a generic expense control account) — Required so that postings sum to zero.
...
- Create postings:
  - Posting 1: Expense/Income ledger posting (synthetic control account) — amount = +X for debit or −X for credit, depending on sign convention.
...
- Create journal_entry:
  - For Buy:
    - Debit: Investment holding (synthetic asset ledger) — cost basis
    - Credit: Funding Account (bank) — amount paid (units\*price + fees)
  - Also create posting for fees to Fee ledger if fees exist.
  - For Sell:
    - Debit: Funding Account (bank) — proceeds
    - Credit: Investment holding — cost basis (realized portion)
...
**Tables: Separate balance tables per account type**

- **bank_account_balances**: account_id (PK), balance_amount, last_posting_id, updated_at
- **credit_card_balances**: account_id (PK), balance_amount, last_posting_id, updated_at
- Purpose: Fast reads for account list, dashboard, net-worth calc.

**Update strategy**

- On every successful journal entry creation (commit), update balance table for affected accounts atomically.
- Use DB transaction + SELECT ... FOR UPDATE to avoid race conditions.
- LedgerService.\_update_account_balance() handles both BankAccount and CreditCard.
- For restoration, re-compute balances by replaying postings from ledger (maintenance tool).
- Management command: `python manage.py recalculate_balances --dry-run`

**Net worth**

- Net worth calculation uses `get_current_balance()` method on each account
- Iterates through active bank accounts and sums balances
- Falls back to opening_balance if balance record missing (graceful degradation)
- Does NOT include credit card balances (debt is tracked separately)
- Formula: `sum(account.get_current_balance() for account in BankAccount.objects.filter(status='active'))`

**Balance Integrity**

- Verification: `opening_balance + SUM(postings)` should equal `balance_amount`
- Recalculation command rebuilds balance records from ledger
- --cleanup-orphans flag removes orphaned journal entries
- All journal entries must balance to zero (validated at creation)
...
10. Django app & module layout (recommended — modular B)

**Implemented Django apps and responsibilities:**

- **core/** — Shared utilities, middleware, BaseAccount model, template tags (indian_numbers)
- **accounts/** — BankAccount model (savings/checking/current/salary), views, forms, balance tables
- **creditcards/** — CreditCard model, views, forms, credit card balances (COMPLETED)
- **authn/** — Sign up/login/logout, password change
- **transactions/** — Transaction model (GenericFK to accounts), unified view with transfers tab
- **ledger/** — JournalEntry, Postings, LedgerService, ControlAccounts, recalculate_balances command
- **transfers/** — Transfer model and forms, integrated into transaction_list view
- **categories/** — Category management (hierarchical, income/expense types)
- **investments/** — Placeholder (future)
- **fds/** — Placeholder (future)
- **loans/** — Placeholder (future)
- **cash/** — Placeholder (future)
- **wallets/** — Placeholder (future)
- **reports/** — Placeholder (future)
- **activity/** — ActivityLog model, tracking utilities, change tracking
- **ui/** — Common page components, templates

**Template Tag Libraries:**

- **indian_numbers** (core/templatetags/) — Indian number formatting with decimal control
- **transaction_tags** (transactions/templatetags/) — Account display with emoji indicators

**APIs / Views**

- Use Django class-based views + endpoint style URLs for table AJAX.
- Example endpoints:
  - GET /transactions/ — transactions page (paginated)
  - POST /transactions/new/ — create transaction (form)
  - POST /transfers/new/ — create transfer (form)
...
- Datetimes: store in timestamp with time zone with application always writing IST; recommended to store naive or timezone-aware consistently—per your choice you requested IST stored; so store timestamp without time zone but document that server timezone must be IST. (Alternative recommended: store UTC and translate to IST on display—however you insisted store in IST for easier querying; we will store IST timestamp and ensure DB timezone is configured accordingly.)

**Indexes (recommended)**

- transactions(user_id, account_id, date) — composite index for filtering lists.
- transactions(user_id, date, amount) — for searches.
- postings(journal_entry_id), postings(account_id) — for balance replays.
- account_balances(account_id) — primary key.
...
**Transfer Validation (Three-Layer Approach):**

1. **Form Validation (transfers/forms.py):**

   - Validates from_account ≠ to_account
   - Compares ContentType ID AND object ID (allows BankAccount ID=4 → CreditCard ID=4)
   - Checks bank account sufficient balance (credit cards exempt)
   - Amount must be > 0

2. **Model Validation (transfers/models.py):**

   - Uses `_id` suffix for ContentType comparison (more efficient)
   - Same account check: `from_account_content_type_id == to_account_content_type_id AND from_account_object_id == to_account_object_id`
   - skip_validation parameter for programmatic saves

3. **Service Validation (ledger/services.py):**
   - Gets ContentType for each account and compares IDs
   - Ensures consistency across all validation layers
   - Atomic balance updates with select_for_update()

**Insufficient Balance Validation:**

- Bank accounts: Cannot go negative, expenses validated against current balance
- Credit cards: Can go more negative (no validation), debt increases with expenses
- User-friendly error messages with current balance and requested amount
- Applied in both TransactionForm and TransferForm

**Phase 3F Comprehensive Integration Testing:**

Completed 8 test suites covering:

- Transaction Tests (income/expense on credit cards)
- Transfer Tests (bank↔credit card, credit card↔credit card)
- Dashboard Tests (stats, modals, recent transactions)
- Accounts & Cards Page Tests (combined view, responsive design)
- Account Detail Pages (transaction/transfer history)
- Deletion Protection (prevent deletion with dependencies)
- Balance Recalculation (ledger integrity verification)
- Edge Cases (overpayment, decimals, large numbers, formatting)

Key findings:

- Fixed GenericForeignKey comparison bug (ContentType + object ID)
- Fixed balance calculation to use get_current_balance() method
- Verified Indian number formatting across all pages
- Confirmed decimal precision (2 decimal places)
- All journal entries balanced to zero ✓
- No orphaned journal entries ✓
- Balance integrity verified ✓

14. Scaling, backups & deployment (Docker)

**Docker Compose (V1)**

- web: Django app (Gunicorn) — serves dynamic pages and AJAX endpoints.
- db: PostgreSQL 16 — persists data in volume /var/lib/postgresql/data.
...
- For the requested small scale (2–3 users, thousands of transactions), single Postgres + single web container is sufficient.
...
- Transfers create balanced postings and update both accounts’ balances.
...
- Store datetimes in IST per your request for easier querying—ensure server timezone alignment.
