...
class CreditCard(BaseAccount):
    ...
    def get_current_balance(self):
        """
        Get current balance from CreditCardBalance materialized table.
        ...
        Important: For credit cards, balance semantics are inverted:
        - Negative balance = Amount owed (liability/debt)
        - Zero balance = No debt
        - Positive balance = Overpayment (rare, but possible)

        Returns:
            Decimal: Current balance from materialized table,
                    or opening_balance if balance record missing

        Example:
            Balance -5000.00 means you owe ₹5,000 to the credit card company
        """
        try:
    ...
    def available_credit(self):
        ...
        # Example: limit=50000, balance=-5000 → 50000 + (-5000) = 45000 available
        # If balance is positive (overpayment), available = limit + balance
        # Example: limit=50000, balance=+1000 → 50000 + 1000 = 51000 available
